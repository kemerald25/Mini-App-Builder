...
export class MiniAppGenerator {
    ...
    determineAppType() {
        if (this.config.needsAgent)
            return 'agent-integrated';
        if (this.config.needsTransaction)
            return 'transaction';
        if (this.config.category === 'gaming')
            return 'game';
        if (this.config.features.some(f => f.toLowerCase().includes('poll')))
            return 'poll';
        if (this.config.features.some(f => f.toLowerCase().includes('nft')))
            return 'nft-gallery';
        return 'simple';
    }
    generatePage() {
