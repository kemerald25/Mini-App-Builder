...
export default function Home() {
  ...
  const { userData, signIn, signOut, isAuthenticated, isLoading } = useQuickAuth();
          ...
          <div className="flex min-h-screen items-center justify-center px-4 pb-32">
            <div className="w-full max-w-[380px] space-y-8">
              <div className="text-center space-y-4">
                ...
                <p className="text-sm text-[#A0A0A0]">A Dev Royale experience tailored for Base builders</p>
              </div>
              <div className={`${cardClasses} text-center space-y-6`}>
                ...
                <p className="text-sm text-[#A0A0A0]">
                  Connect your wallet or Farcaster identity to unlock the full Royale experience.
                </p>
                <div className="space-y-3">
                  <button onClick={signIn} disabled={isLoading} className={primaryButtonClasses}>
                    {isLoading ? 'Connecting…' : 'Connect Wallet'}
                  </button>
                  <button disabled className="w-full text-xs text-[#A0A0A0]">
                    Secure by WalletConnect • Powered by Base
                  </button>
                </div>
              </div>
              <BrandFooter />
      ...
      <main className={containerClasses}>
        <div className={shellClasses}>
          <header className="space-y-4 border-b border-[#1E1F25] pb-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-[#A0A0A0]">Everything you need to track your Base presence</p>
              </div>
          ...
          <section className={`${cardClasses} space-y-3`}>
            ...
            {['Start Quest', 'Invite Friends', 'Explore Base'].map((label) => (
              <button
                key={label}
