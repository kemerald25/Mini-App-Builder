...
import { User, Wallet, Award, Activity, Check } from 'lucide-react';
...
export default function Profile() {
  ...
  const displayName = user?.displayName || 'Royale User';
      ...
      <main className={containerClasses}>
        <div className={shellClasses}>
          <header className="space-y-3 border-b border-[#1E1F25] pb-6">
            ...
            <p className="text-sm text-[#A0A0A0]">Manage your Farcaster credentials and Royale presence.</p>
          </header>
            ...
            {userData?.address && (
              <div className="rounded-2xl border border-[#1E1F25] bg-[#0F1116] p-4">
                <div className="flex items-center justify-between text-xs text-[#A0A0A0]">
                  ...
                  <button
                    onClick={copyAddress}
                    className="rounded-lg border border-[#1E1F25] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-[#A0A0A0] transition-colors duration-200 hover:border-[#0052FF]/40"
                  >
                    {copied ? 'Copied' : 'Copy'}
                  </button>
                </div>
